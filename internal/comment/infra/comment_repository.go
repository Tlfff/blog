...
// NewCommentRepository 创建 Comment 上下文的 GORM Repository。
...
// CreateWithCounts 创建评论，并在同一事务内维护主评论回复数与文章评论数。
func (r *commentRepository) CreateWithCounts(ctx context.Context, comment *domaincommunity.Comment, incrementReply bool) error {
	...
	if incrementReply {
		...
		if err := toDomainComment(rootComment).EnsureReplyable(); err != nil {
			return err
		}
	}
...
// FindByID 按 ID 查询单条评论，未命中时返回领域错误。
...
// ListRootComments 分页查询文章的主评论列表，同时联表补齐作者与被回复者信息。
//
// 参数说明：
...
// CountRootComments 统计文章的主评论总数，支持只看楼主。
...
// DeleteWithCounts 删除评论，并在同一事务内维护主评论回复数与文章评论数。
...
// findByIDForUpdate 在事务内加行锁查询评论，避免并发回复与删除产生计数错乱。
...
// updateCommentCountDelta 按增量更新主评论的回复数。
...
// batchUpdateChildCommentStatus 批量把指定主评论下的正常回复标记为已删除。
...
// updateStatus 把指定评论从正常状态更新为目标状态，并返回受影响行数。
...
// toModelComment 把评论领域对象转换为数据库模型。
...
// toDomainComment 把数据库模型转换为评论领域对象。
...
// toDomainCommentWithUsers 把联表查询结果批量转换为带用户信息的评论领域对象。
