...
func (s *Service) CreateComment(ctx context.Context, articleID, rootID, userID, replyToUserID uint64, content, ip string) (*commentdto.CreateCommentResponse, error) {
	if s.articles != nil {
		exists, err := s.articles.Exists(ctx, articleID)
		if err != nil {
			return nil, err
		}
		if !exists {
	...
	if s.events != nil {
		logError("发布评论事件失败", s.events.PublishCommentCreated(ctx, domaincommunity.CommentCreated{EventID: uuid.NewString(), Version: 1, UserID: userID, ArticleID: articleID, RootID: rootID, CreatedTime: comment.CreatedTime}))
	}
	// 3. 返回新评论ID与创建时间
