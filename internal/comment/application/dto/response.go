...
// 构造主评论列表响应
func NewRootCommentListResponse(models []*domain.Comment, userMap map[uint64]*CommentUserInfo, total int64, lastID, page, page_size uint64, likeMap map[uint64]uint64) *RootCommentListResponse {
	// 1. 初始化响应容器，回填分页信息
...
func NewReplyListResponse(models []*domain.Comment, userMap map[uint64]*CommentUserInfo, total int64, lastID, page, page_size uint64, likeMap map[uint64]uint64) *ReplyListResponse {
	// 1. 初始化响应容器，回填分页信息
