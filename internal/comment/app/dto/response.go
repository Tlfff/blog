...
// 参数说明：
//   - comments：评论领域对象列表。
//   - userMap：用户唯一标识到公开信息的映射。
//   - total：主评论总数。
//   - lastID：下一页查询使用的游标评论 ID。
//   - page：当前页码。
//   - pageSize：每页数量。
//   - likeMap：评论唯一标识到点赞数的映射。
func NewRootCommentListResponse(comments []*domain.Comment, userMap map[uint64]*CommentUserInfo, total int64, lastID, page, pageSize uint64, likeMap map[uint64]uint64) *RootCommentListResponse {
	// 1. 初始化响应容器，回填分页信息
	...
	for _, comment := range comments {
		...
		userInfo, exists := userMap[comment.UserID]
		if !exists {
			userInfo = &CommentUserInfo{UserID: comment.UserID, Username: "未知用户", Avatar: ""}
		}
		// 从redis中获取评论对应点赞数
		likeCount, ok := likeMap[comment.ID]
		if !ok {
		...
		resp.List = append(resp.List, &RootCommentItem{
			...
			CreatedTime: comment.CreatedTime.Unix(),
			Status:      comment.Status,
			IP:          ip.ConvertIPToRegion(comment.IP),
...
//   - pageSize：每页数量。
//   - likeMap：评论唯一标识到点赞数的映射。
func NewReplyListResponse(comments []*domain.Comment, userMap map[uint64]*CommentUserInfo, total int64, lastID, page, pageSize uint64, likeMap map[uint64]uint64) *ReplyListResponse {
	// 1. 初始化响应容器，回填分页信息
	...
	for _, comment := range comments {
		...
		if comment.ReplyToUserID > 0 {
			if targetUser, ok := userMap[comment.ReplyToUserID]; ok {
				replyToUserInfo = targetUser
			} else {
				replyToUserInfo = &CommentUserInfo{UserID: comment.ReplyToUserID, Username: "未知用户", Avatar: ""}
			}
		}
		// 从redis中获取评论对应点赞数
		likeCount, ok := likeMap[comment.ID]
...
// NewCommentStatsResponse 构造评论统计信息响应。
