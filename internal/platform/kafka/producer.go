...
func NewProducer(cfg config.Kafka) (*Producer, error) {
	...
	for topicKey, topicCfg := range cfg.Topics {
		...
		// 2.1 验证 topic 名称是否为空
		if topicCfg.Name == "" {
			return nil, apperrors.ErrKafkaTopicNotConfig
		}
...
func (p *Producer) sendMessage(ctx context.Context, topicKey string, key []byte, value interface{}) error {
	...
	if !exists {
		return apperrors.ErrKafkaTopicNotConfig
	}
	// 2. 序列化消息
	data, err := json.Marshal(value)
	if err != nil {
		return apperrors.ErrKafkaSerializeFailed
	}
	// 3. 构建消息
	msg := kafka.Message{
...
func (p *Producer) Close() error {
	...
	// 2. 检查是否有错误
	if len(errs) > 0 {
		return apperrors.ErrKafkaCloseFailed
	}
	return nil
}
