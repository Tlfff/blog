...
func TestUserHandler_AllRoutes(t *testing.T) {
	...
	// 把真实组装好的服务喂给 Handler
	h := NewUserHandler(userService, userAuthService)
	...
	tests := []struct {
		...
		// ====================  场景 B：查看他人主页 (GetPublicProfile) ====================
		{
			...
			ctxUser:      nil,
			expectStatus: http.StatusBadRequest, // 如果进入 c.Error(apperrors.ErrUserNotFound)
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(c.Errors) > 0 {
				...
				if err == apperrors.ErrInvalidRequestBody || err == apperrors.ErrUserNotFound {
