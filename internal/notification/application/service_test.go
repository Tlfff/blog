...
func TestCreateArticleLikeNotification(t *testing.T) {
	...
	if err := service.CreateLikeNotification(context.Background(), domain.NotificationEvent{
		NotifyType: domain.NotifyTypeLikeArticle, SenderID: 2, TargetID: 3, CreatedTime: createdTime,
	}); err != nil {
...
func TestRejectsUnwiredNotificationType(t *testing.T) {
	...
	if err := service.CreateLikeNotification(context.Background(), domain.NotificationEvent{NotifyType: domain.NotifyTypeLikeComment}); err == nil {
		t.Fatal("未接线通知类型应被拒绝")
	}
}
