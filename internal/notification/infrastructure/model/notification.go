...
// CommentNotifyContent 是评论类通知内容的 MongoDB 嵌入模型。
type CommentNotifyContent struct {
	ArticleID uint64 `bson:"article_id"` // 文章唯一标识
	CommentID uint64 `bson:"comment_id"` // 评论唯一标识
}

// 消息主文档
type Notification struct {
	ID          primitive.ObjectID `bson:"_id,omitempty" ` // MongoDB 文档主键
	EventID     string             `bson:"event_id" `      // 来源业务事件唯一标识
