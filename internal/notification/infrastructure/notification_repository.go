...
import (
	domaincommunity "blog/internal/notification/domain"
...
func (r *notificationRepository) Insert(ctx context.Context, notification *domaincommunity.Notification) error {
	if notification.EventID != "" {
		...
		err := r.db.Collection("notifications").FindOne(ctx, bson.M{"event_id": notification.EventID}).Decode(&existing)
		if err == nil {
			notification.ID = existing.ID.Hex()
			return nil
		}
...
func toModelNotification(n *domaincommunity.Notification) *model.Notification {
	...
	if comment, ok := n.Content.(domaincommunity.CommentContent); ok {
		content = model.CommentNotifyContent{ArticleID: comment.ArticleID, CommentID: comment.CommentID}
	}
	// 3. 组装 Mongo 文档模型
...
func toDomainNotification(m *model.Notification) *domaincommunity.Notification {
	...
	if content, ok := m.Content.(model.CommentNotifyContent); ok {
		n.Content = domaincommunity.CommentContent{ArticleID: content.ArticleID, CommentID: content.CommentID}
	}
	return n
}
