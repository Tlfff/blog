...
import (
	domaincommunity "blog/internal/like/domain"
...
type articleLikeRepository struct {
	...
	statistics domaincommunity.ArticleLikeStatistics // Article 统计 Port
}

// NewArticleLikeRepository 返回直接持有 GORM 的文章点赞 Repository 实现。
func NewArticleLikeRepository(db *gorm.DB, statistics ...domaincommunity.ArticleLikeStatistics) domaincommunity.ArticleLikeRepository {
	var stats domaincommunity.ArticleLikeStatistics
	if len(statistics) > 0 {
		stats = statistics[0]
	}
	return &articleLikeRepository{db: db, statistics: stats}
}
...
func (r *articleLikeRepository) SetLiked(ctx context.Context, userID, articleID uint64, liked bool) error {
	...
	if r.statistics != nil {
		return r.statistics.IncrementLikeCount(ctx, articleID, int64(delta))
	}
	return nil
}
...
type commentLikeRepository struct {
	db         *gorm.DB                              // GORM 数据库连接
	statistics domaincommunity.CommentLikeStatistics // Comment 统计 Port
}

// NewCommentLikeRepository 返回直接持有 GORM 的评论点赞 Repository 实现。
func NewCommentLikeRepository(db *gorm.DB, statistics ...domaincommunity.CommentLikeStatistics) domaincommunity.CommentLikeRepository {
	var stats domaincommunity.CommentLikeStatistics
	if len(statistics) > 0 {
		stats = statistics[0]
	}
	return &commentLikeRepository{db: db, statistics: stats}
}
...
func (r *commentLikeRepository) SetLiked(ctx context.Context, userID, commentID uint64, liked bool) error {
	...
	if r.statistics != nil {
		return r.statistics.IncrementLikeCount(ctx, commentID, int64(delta))
	}
	return nil
}
