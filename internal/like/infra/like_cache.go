...
import (
	...
	redisUtil "blog/internal/platform/redis"
	"context"
	"log"
	"strconv"
	"time"
...
const (
	articleLikePrefix      = "like:article:"       // 文章点赞 Set Key 前缀
	commentLikePrefix      = "like:comment:"       // 评论点赞 Set Key 前缀
	articleLikeLockPrefix  = "lock:like:article:"  // 文章点赞缓存重建锁前缀
	commentLikeLockPrefix  = "lock:like:comment:"  // 评论点赞缓存重建锁前缀
	commentLikeCountPrefix = "like:comment:count:" // 评论点赞数兼容 Key 前缀
	likeLockExpire         = 3 * time.Second       // 点赞缓存重建锁过期时间
	likeCacheExpire        = 7 * 24 * time.Hour    // 点赞 Set 缓存过期时间
)
...
// IsLiked 查询用户是否点赞目标对象，缓存未命中时加锁重建缓存。
func (a *likeCacheAdapter) IsLiked(ctx context.Context, target domaincommunity.LikeTarget, targetID, userID uint64) (bool, error) {
	...
	lock := redisUtil.NewRedisLock(a.rdb, lockKey, likeLockExpire)
...
func (a *likeCacheAdapter) rebuildAndCheck(ctx context.Context, target domaincommunity.LikeTarget, targetID, userID uint64) (bool, error) {
	...
	if err := a.rdb.SAdd(ctx, key, members...).Err(); err != nil {
		...
		a.rdb.Expire(ctx, key, likeCacheExpire)
	}
	return isLiked, nil
}
...
// GetCommentLikeCounts 批量读取评论点赞数缓存。
...
func likeKey(target domaincommunity.LikeTarget, targetID uint64) string {
	prefix := articleLikePrefix
	if target == domaincommunity.LikeTargetComment {
		prefix = commentLikePrefix
	}
...
func likeLockKey(target domaincommunity.LikeTarget, targetID uint64) string {
	prefix := articleLikeLockPrefix
	if target == domaincommunity.LikeTargetComment {
		prefix = commentLikeLockPrefix
	}
	return prefix + strconv.FormatUint(targetID, 10)
}
