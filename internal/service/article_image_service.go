...
// ArticleImageService 负责文章内嵌图片的直传凭证生成
...
// NewArticleImageService 注入 MinIO 客户端、公开域名和允许的扩展名
...
func (s *ArticleImageService) GetUploadURL(ctx context.Context, fileExt string) (uploadURL, objectKey, url string, err error) {
	if s.oss == nil {
	...
	return uploadURL, objectKey, url, nil
}
