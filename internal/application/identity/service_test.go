...
func TestService_ChangePasswordInvalidatesOtherSessions(t *testing.T) {
	...
	if _, err := s.Login(ctx, "13900000000", "", "oldpass", "127.0.0.1", "web", false); !errors.Is(err, common.ErrPasswordFailed) {
...
func TestService_AvatarContract(t *testing.T) {
	...
	if _, _, err := s.GetAvatarUploadURL(ctx, 1, "exe"); !errors.Is(err, common.ErrInvalidRequestBody) {
	...
	if _, err := s.ConfirmAvatar(ctx, 1, "avatar/2/hack.png"); !errors.Is(err, common.ErrInvalidRequestBody) {
